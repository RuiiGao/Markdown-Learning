# 一级标题

## 二级标题

### 三级标题

#### 四级标题
##### 五级标题
###### 六级标题
---

## 基础语法
**无序列表:**
- 无序列表1
+ 无序列表2
* 无序列表3
* 无序列表4
  换行也是可以保持对齐的

**有序列表:**
1. 有序列表1
2. 有序列表2
    1. 有序列表3
    2. 有序列表4

**任务列表:**
- [ ] 任务1
- [x] 字母x表示完成
- [ ] 注意是`-空格[空格]空格`和`-空格[x]空格`
- [ ] 也可以在右边的preview中勾选

*斜体*  _斜体_

**加粗**  __加粗__

***粗斜体***  ___粗斜体___

注意：推荐使用星号 `*` 而不是下划线 `_` ，因为星号在各种 Markdown 解析器中兼容性更好

~~删除~~

`内容`

空格：`&nbsp;` 如：我&nbsp;你

==高亮==
- 不支持的话就用`<mark></mark>`标签
- <mark>高亮</mark>

~下标~  
- 不支持的话就用`<sub></sub>`标签
- 下标<sub>下标</sub>

^上标^
- 不支持的话就用`<sup></sup>`标签
- 上标<sup>上标</sup>

<u>这是下划线</u>

换行：两个以上空格后回车/空一行（VS code自带的渲染器预览效果效果需要这么干，而插件的预览直接换行就可以实现）
HTML 换行标签 `<br>`  以及 `\` 也行（部分解析器支持）

横线:
- 三个及以上的-
- 三个及以上的*
- HTML的\<hr>
- 注意前面要空行 不然会把横线上一行的内容渲染成跟二级标题一样大小的东西

没空行
---
***
<hr>

- - -
* * *


## 表格
3个部分:
- 表头
- 对齐方式:短横线:至少有一个
- 内容

如:
| 左对齐 | 居中 | 右对齐 |
| :- | :-: | -: |
|来试试大段文字的对齐状况吧!|也可以加入latex公式,还是使用$\frac{\sqrt[3]{8x}}{9 \pi y}$行内公式|最后是右对齐,就不写啥了|
|还可以新建第二行|后面我就|啥都不写了|
 ---

## 引用

>引用,使用英文大于号>
>多行的引用
>
>>引用也可以嵌套
>>

引用是一个容器，内部可以包含标题、列表、代码块等几乎所有其他 markdown元素
如：
> ### 这是一个标题
>
> 1. 这是列表项一
> 2. 这是列表项二
> - 这是也个列表
>
> 这是一个代码块：
> ```javascript
> console.log("Hello, from a quote!");
> ```

列表中使用区块实例如下：
* 第一项
  >今天
  >>还有明天
* 第二项
---

## 代码块
代码块：`单个反引号(行内代码块，不显示代码高亮)`或者`三个反引号加语言，结束的时候再三个反引号(跨行代码块)`
如：

```python
print('hello markdown')
```

```html {.line-numbers}
<p>hello mark down</p>
```

```mermaid
%%{
    init{'theme':'neutral'}
}%%

graph LR
  id1(这是一段文字)-->ID2(这也是一段文字);
```

用 `diff` 可以展现代码差异对比：
```diff
function calculateTotal(items) {
-   let total = 0;
+   let total = 0.0;
    
    for (let item of items) {
-       total += item.price;
+       total += parseFloat(item.price);
    }
    
+   // 保留两位小数
+   total = Math.round(total * 100) / 100;
    return total;
}
```

还有老版本的代码块，使用一个缩进，不支持代码高亮：

    %%{
        init{'theme':'neutral'}
    }%%

    graph LR
    id1(这是一段文字)-->ID2(这也是一段文字);

其他缩进示例：
正常文本段落

    这是缩进式代码块
    每行前面有四个空格
    保持代码的原始格式
    
继续正常文本

**注意事项**：

- 缩进式代码块前后需要空行分隔
- 所有代码行必须保持一致的缩进
- 不支持语法高亮
- 在列表中使用时需要8个空格缩进

```还有包含 `` 双反引号的代码```
---

## 脚注
- 两部分，链接 + 内容
- 语法：`[^content]`
- 这是范例[^脚注1]
    - 注意，VSCode自带的md渲染器的脚注内容不能有空格！

[^脚注1]:这是脚注在文章最后的对应部分，点击可以跳转
---

## 链接
### 基础用法
语法：`[链接文本](链接地址 "hover时候的提示")`，注意中括号和小括号之间不要有空格，而""前要有空格

- 示例：[谷歌](https://www.google.com.hk/?hl=zh-CN "Google")
- 如果是网址的话，直接粘贴也行，例如：https://www.google.com.hk/?hl=zh-CN 或者用 `<>` 括起来，例如：<https://www.google.com.hk/?hl=zh-CN>

### 文件内转跳
语法：`[显示的文字](#转跳的标题名称)`；注意：标题的名称是需要转写的，规则一般是：**转为小写、用连字符 - 替换空格、移除特殊字符**。

示例：[转跳测试](#代码块) 

### 文件间转跳
- 语法：`[显示的文字](./路径/文件名称.md#转跳的标题名称)`；标题的转写规则同上
- 示例：参见[测试](./test.md#在小部分)
- **注意**：不要使用`MPE`的预览，会报错（但貌似现在修复了...）

### 引用链接
适用于多次出现同一个链接的情况，把后面的小括号换成中括号，在最后统一写中括号的内容；可以类似认为是`链接 + 脚注`
- 注意：链接的语法为`[名称]: 链接地址(前面有空格)`，且其不会在渲染器中显示出来
- 例如：[Google][id]
- 再来一次：[咕噜咕噜][id]

[id]: https://www.google.com.hk/?hl=zh-CN

### 图片链接
- 语法：`![图片无法显示时的替代文本](图片链接或路径)`（就是在链接前加上一个！）
- 例如：![gugesousuo](https://www.google.com.hk/images/branding/googlelogo/2x/googlelogo_color_272x92dp.png)

### 总结
- 如何记住是`[]`内写文字、`()`内写链接？
  - `[]`在视觉上就像一个矩形框，正好适合放“**标签**文本”
  - Markdown中，方括号`[]`几乎总是用来包裹一个“人类可读”的文本、标签或提示符
    - 脚注`[^]`
    - 转跳`[]()`
    - 链接`[][] / []()`
    - 图片`![]()`
- 如何记住是标签在前而链接在后？ 
  - 如果反过来的话，阅读起来其实会非常别扭（“可读性低”）
  - 正确的写法：
    - >我最常访问的网站是`[谷歌搜索](https://www.google.com)`，它非常方便
    - 句子读起来非常通顺：“我最常访问的网站是 谷歌搜索，它非常方便。”
  - 错误的写法：
    - >我最常访问的网站是`(https://www.google.com)[谷歌搜索]`，它非常方便
    - 句子就变成了：“我最常访问的网站是 https冒号斜杠斜杠www点google点com（什么b动静？），它非常方便。”

### 补充内容
- 邮箱（自动识别）：联系邮箱： 3240037847@qq.com  或者 <3240037847@qq.com>
---

## 🥂Emoji✨
- Emoji是被Unicode标准收录的字符，因此，其版本是跟随 Unicode 标准的版本发布的
  - 一个 emoji 能否在你的设备上正确显示，取决于你的操作系统和软件是否支持包含了该 emoji 的 Unicode 版本
- 使用Emoji的几种方法
  1. 🐺直接粘贴：详见网站[Emojipedia](https://emojipedia.org/)
  2. 短代码，语法为`:表情名称:`
     1. （短代码并不是 Markdown 原始标准（CommonMark）的一部分。它的支持情况完全取决于你使用的Markdown渲染器。）
     2. 参考Github的[emoji短代码列表仓库：emoji-cheat-sheet](https://github.com/ikatyang/emoji-cheat-sheet)
     3. 几个示例：:heart:，:smiling_face_with_three_hearts:，:kissing_heart:，:sweat_smile:
  3. 使用Unicode，语法：`&#x代码的十六进制数;`
     1. 只要 Markdown 解析器支持 HTML ，就能正确渲染，与平台无关。
     2. Unicode详见网站 [Full Emoji List](https://unicode.org/emoji/charts/full-emoji-list.html)
     3. 几个示例：&#x1F925;（记得去掉`U+`）
---

## 其他资源
1. Markdown 语法（菜鸟教程）：https://www.runoob.com/markdown/
2. Emoji厨房：https://emojikitchen.dev/
